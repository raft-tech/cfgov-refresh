...
function scriptsPolyfill() {
  return (
    gulp
      .src(paths.unprocessed + "/js/routes/common.js")
      .pipe(
        gulpNewer({
      ...
      .pipe(
        gulpModernizr("modernizr.min.js", {
...
function scriptsNonResponsive() {
  return gulp
    .src(paths.unprocessed + "/js/routes/on-demand/header.js")
    .pipe(
      gulpNewer({
...
function scriptsMyMoneyCalendar(done) {
  const apps = [`${paths.unprocessed}/apps/mmt-my-money-calendar`];
  const streams = [];
...
function scriptsCostOfCreditTool(done) {
  const apps = [`${paths.unprocessed}/apps/mmt-cost-of-credit-tool`];
  const streams = [];
...
gulp.task("scripts:costofcredittool", scriptsCostOfCreditTool);
gulp.task("scripts:mymoneycalendar", scriptsMyMoneyCalendar);
